...
public class ItemServiceImpl implements ItemService {
	...
	/**
	 * 自动注入 TbItemMapper 接口的动态代理实现类对象
	 */
	@Autowired
	private TbItemMapper itemMapper;

	/**
	 * 自动注入 TbItemDescMapper 接口的动态代理实现类对象
	 */
	@Autowired
	private TbItemDescMapper itemDescMapper;

	/**
	 * 自动注入 TbItemParamItemMapper(商品规格参数) 接口的动态代理实现类对象
	 */
	@Autowired
	private TbItemParamItemMapper itemParamItemMapper;

	/**
	 * 读取配置文件中的值，商品信息在 Redis 中保存的 key
	 */
	@Value("${REDIS_ITEM_KEY}")
	private String REDIS_ITEM_KEY;

	/**
	 * 商品信息在 Redis 中保存的时间，60*60*24=86400 (一天时间)
	 */
	...
	/**
	 * 接收商品id，根据商品id查询商品描述。返回商品描述的pojo 
	 * 	描述： 
